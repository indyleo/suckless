...
static void readflatpak(void) {
  FILE *fp;
  char line[1024];
  size_t i, itemsiz = 0;

  /* count existing items to find array end */
  if (items)
  ...
  /* pre-size: find current alloc size (approximate — just grow from i) */
  itemsiz = i + 256;
  if (!(items = realloc(items, itemsiz * sizeof(*items))))
    die("cannot realloc %zu bytes:", itemsiz * sizeof(*items));

  fp = popen("flatpak list --app --columns=name,application 2>/dev/null", "r");
  if (!fp)
    return;
  ...
  /* re-clamp lines now that we have more items */
  /* lines global is already set; just leave it — setup() calls match() */
}
